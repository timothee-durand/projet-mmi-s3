<template>
  <div>
    <SidebarClient sidebarWidth="15%">
      <div class="p-2 d-flex flex-column align-items-center">
        ...
        <span v-if="contientMaterielPro" v-bind:class="{'text-white': true, activeStepper: checkifStateIs('motivation')}">Motivation</span>
        ...
        <div class="border-left border-white mt-2 mb-2 separator"></div>
        <span v-bind:class="{'text-white': true, activeStepper: checkifStateIs('lieu')}">Où récupérer mon matériel ?</span>
        <div class="border-left border-white mt-2 mb-2 separator"></div>
      </div>
    </SidebarClient>
    ...
    <div v-if="checkState === 'selection'" class="containerRight">
        ...
        <div v-for="materiel in this.selectedMateriel" :key="materiel.id" class="c-card p-3 col-12 mt-3 shadow"
          ...
          <div class="d-flex flex-row align-items-center justify-content-between cardContent">
            ...
            <div class="cardDesc usageView" style="overflow: scroll" v-html="materiel.usage"></div>
              ...
              <img v-if="materiel.pro === 1" src="https://placekitten.com/20/20" alt="premimum" class="d-block ml-auto mr-0 float-right" style="width: 25px; align-self: start">
          ...
          <div v-for="malette in this.selectedMalette" :key="malette.id" class="c-card p-3 col-12 mt-3 shadow"
              ...
              <div class="d-flex flex-row align-items-center justify-content-between cardContent">
                  ...
                  <b-button v-on:click="deleteMaletteFromReservation(malette.id)" variant="white" v-b-tooltip.hover.left title="Retirer de la réservation" class="mr-0 ml-auto p-0"><i class="fas fa-trash text-secondary"></i></b-button>
      ...
      <div class="position-fixed" style="right:40px; bottom: 40px">
        <b-button v-if="this.selectedMateriel !== null && this.selectedMateriel.length > 0 && contientMaterielPro" pill variant="primary" @click="$router.push('motivation')">Continuer</b-button>
        <b-button v-if="this.selectedMateriel !== null && this.selectedMateriel.length > 0 && !contientMaterielPro" pill variant="primary" @click="$router.push('rendez-vous')">Continuer</b-button>
      </div>
      <div v-if="this.selectedMateriel === null || this.selectedMateriel.length <= 0" style="height: 65vh;" class="w-100 d-flex align-items-center justify-content-center text-center">
          <p>Vous n'avez ajouté aucun contenu à votre réservation.<br>Vous pouvez en ajouter depuis la <router-link to="/search/Tous" style="text-decoration: underline" class="text-dark" >recherche.</router-link></p>
      </div>
    </div>
    </div>
    ...
    <div v-else-if="checkState === 'rendez-vous'" class="containerRight p-5 d-flex justify-content-center align-items-center">
      <div class="c-card windowCard shadow p-5">
        ...
        <label class="mr-2" >{{new Date($store.getters.getReservdateDebut).toLocaleDateString("fr-FR", {weekday: "long", month: "long", day: "2-digit"})}} --</label>
        <b-form-select
        ...
        <div v-for="(date, index) in getNextDays()" :key="index" class="w-100 d-flex flex-row">
            <b-form-radio name="dayRadios" v-model="dayRadio" :value="index" class="mb-2 mr-sm-2 mb-sm-0 d-flex align-items-center w-25">
              <div class="mr-2" >{{date.toLocaleDateString("fr-FR", {weekday: "long", month: "long", day: "2-digit"})}} --</div>
            </b-form-radio>
            <b-form-select
                    id="inline-form-time-picker"
                    class="mb-2 mr-sm-2 mb-sm-0"
        ...
        <b-button v-if="contientMaterielPro" variant="primary" pill class="position-fixed" style="right:150px; bottom: 40px"
        ...
        <b-button v-if="dayRadio !== null" variant="primary" pill class="position-fixed" style="right:40px; bottom: 40px"
                  @click="$router.push('lieu')">
          Continuer
        </b-button>
          <b-button v-b-tooltip.hover title="Merci de selectionner une date de retour et des horaires appropriés" v-else disable pill class="position-fixed" style="right:40px; bottom: 40px">
          Continuer
        </b-button>
      </div>
    </div>
  ...
  <div v-else-if="checkState === 'Envoyé'" class="containerRight  p-5 d-flex justify-content-center align-items-center">
  ...
  methods:
  {
    ...
    getMateriel() {
      ...
      getNextDays()
      {
          ...
          for( let x = 0; x < 3; x++ )
          {
              while( utilsServices.addDays(reservdate,x+i+1).getDay() === 0 )
              {
                  i++;
              }
